...
class VoxelMap:
    ...
    def __init__(
        self, clouds: List[o3d.geometry.PointCloud], poses: List, voxel_size: float
    ) -> None:
        self.number_of_poses = len(poses)

        if len(clouds) != self.number_of_poses:
            raise ValueError(
                f"Length of poses sequence has to be equal to length of clouds ({self.number_of_poses} != {len(clouds)})"
            )
    ...
    def adaptive_feature_extraction(
        self, ransac_distance_threshold: float, adaptive_voxel_size: float
    ) -> Dict[Point3D, Dict[int, VoxelPoints]]:
        ...
        while has_breakable_voxels:
            ...
            EmptyVoxelsFilter(min_voxel_poses=self.number_of_poses).filter(
                voxel_feature_map
            )
            inconsistent_voxels = self._find_inconsistent_voxels(voxel_feature_map)
            has_breakable_voxels = self._break_map_on_octants(
                inconsistent_voxels, adaptive_voxel_size
            )
    ...
    def _break_map_on_octants(
        self, inconsistent_voxels: List[VoxelKey], adaptive_voxel_size: float
    ) -> bool:
        ...
        for voxel_key in inconsistent_voxels:
            ...
            octant_centroids = voxel_utils.get_bounding_box(
                voxel_center=voxel_key.centroid, voxel_size=octant_size
            )
            ...
            for pose_id, voxel_points in self._voxel_to_pose_points_map[
                voxel_key
            ].items():
                for point, point_id in zip(voxel_points.points, voxel_points.pcd_idx):
                    # Find point's octant
                    for oct_center in octant_centroids:
                        if voxel_utils.point_is_in_box(
                            point, voxel_utils.get_bounding_box(oct_center, octant_size)
                        ):
                            octant_key = VoxelKey(oct_center, octant_size)
                            octo_points = self._voxel_to_pose_points_map[
                                octant_key
                            ].get(pose_id, VoxelPoints([], []))
                            octo_points.add_point(point, point_id)
                            self._voxel_to_pose_points_map[octant_key].update(
                                {pose_id: octo_points}
                            )
    ...
    def _find_inconsistent_voxels(
        self, voxel_feature_map: Dict[Point3D, Dict[int, VoxelPoints]]
    ) -> List[VoxelKey]:
